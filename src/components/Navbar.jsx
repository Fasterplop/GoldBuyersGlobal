          ...
          <div className="flex">
            ...
            <ul className="hidden sm:flex items-center space-x-6">
              {navLinks.map((nav) => (
                <li
                  key={nav.id}
                  className=" hover:font-semibold hover:bg-[#FFD700] active:ring-2 font-montserrat py-4 text-black text-[18px] transition duration-300"
                >
                  <Link href={`${nav.id}`}>
                    <span className="py-4 px-3">{nav.title}</span>
                  </Link>
            ...
            <div className="pl-6 items-center space-x-1 sm:flex hidden">
              <a
      ...
      <div className={`${isToggled ? "flex" : "hidden"} `}>
        <ul className="sm:hidden w-full">
          {navLinks.map((nav) => (
            <li
              key={nav.id}
              className="text-black font-montserrat text-center border-b-2 block py-2 px-4 hover:font-semibold hover:bg-[#FFD700] active:ring-2 text-md"
            >
